...
$(function () {
  ...
  function loadBudgetCodes(jenis) {
    return $.getJSON("/api/budget-codes?page=1&pageSize=2000").then(
      function (res) {
        ...
        setTimeout(function () {
          ...
          $sel
            ...
            .css({ "min-height": "38px", padding: "4px 8px" });
        }, 100);
      },
    );
  }
  ...
  function loadCoaTexts(jenis) {
    ...
    if (
      !budgetCodeIds ||
      (Array.isArray(budgetCodeIds) && budgetCodeIds.length === 0)
    ) {
      $sel.append(
    ...
    const validGuids = budgetCodeIds.filter(
      (x) => typeof x === "string" && x.length === 36,
    );
    ...
    return $.getJSON(url).then(function (res) {
      ...
      items.forEach(function (v) {
        $sel.append(
          `<option value="${v.vendorCategoryId}">${v.parentBudgetCodeLabel || "-"} - ${v.name}</option>`,
        );
      });
      $sel.prop("multiple", jenis !== "Advanced");
      $sel.select2({
  ...
  function loadStatusOptions(jenis) {
    ...
    return $.getJSON(url).then(function (res) {
      ...
      items.forEach(function (s) {
        const id =
          jenis === "Advanced" ? s.advancedStatusId : s.biayaRealisasiStatusId;
  ...
  uangMukaTable = $("#unagMukasTable").DataTable({
    ...
    initComplete: function () {
      const $target = $("#uangMukaExportActions");
      if ($target.length) {
        $target.empty();
        $(uangMukaTable.buttons().container()).appendTo($target);
      }
    },
    buttons: [
      {
  ...
  $("#jenis").on("change", function () {
    ...
    if ($("#budgetCodeId").hasClass("select2-hidden-accessible"))
      $("#budgetCodeId").select2("destroy");
    if ($("#coaTextId").hasClass("select2-hidden-accessible"))
      $("#coaTextId").select2("destroy");
        ...
        Promise.all([
          ...
          .then(function () {
            ...
            if (data.jenis === "Advanced") {
              let bc = Array.isArray(data.budgetCodeIds)
                ? data.budgetCodeIds[0]
                : data.budgetCodeId || "";
              $("#budgetCodeId")
                .val(bc ? [bc] : [""])
                .trigger("change");
          ...
          .then(function () {
            ...
            if (data.jenis === "Advanced") {
              let ct = Array.isArray(data.coaTextIds)
                ? data.coaTextIds[0]
                : data.coaTextId || "";
              $("#coaTextId")
                .val(ct ? [ct] : [""])
  ...
  $("#uangMukaForm").on("submit", function (e) {
    ...
    let payload = {
      ...
      uangMukaRelatedId:
        jenis === "Realisasi" ? $("#UangMukaRelatedId").val() : null,
    };
    if (jenis === "Advanced") {
      // Single select, kirim array berisi satu (atau null)
      payload.budgetCodeIds =
        budgetCodeVal && budgetCodeVal !== "" ? [budgetCodeVal] : [];
      payload.coaTextIds = coaTextVal && coaTextVal !== "" ? [coaTextVal] : [];
    } else {
      // Multiple select
      payload.budgetCodeIds = Array.isArray(budgetCodeVal)
        ? budgetCodeVal
        : budgetCodeVal
          ? [budgetCodeVal]
          : [];
      payload.coaTextIds = Array.isArray(coaTextVal)
        ? coaTextVal
        : coaTextVal
          ? [coaTextVal]
          : [];
