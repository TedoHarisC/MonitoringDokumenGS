...
(function ($) {
  ...
  function openUserModal(mode, row) {
    ...
    $("#passwordHint").text(
      isEdit
        ? "Optional for edit (leave blank to keep current)."
        : "Required for create.",
    );
    ...
    const vendorOptions = cachedVendors.map(
      (v) =>
        `<option value="${v.vendorId}">${escapeHtml(v.vendorName)}</option>`,
    );
    setSelectOptions(vendorSelect, vendorOptions, "-- None --");

    const roleOptions = cachedRoles.map(
      (r) =>
        `<option value="${r.roleId}">${escapeHtml(r.code)} — ${escapeHtml(r.name || "")}</option>`,
    );
    setSelectOptions(roleSelect, roleOptions, "-- Select Role --");

    if (isEdit) {
      const vid =
        row.vendorId && row.vendorId !== "00000000-0000-0000-0000-000000000000"
          ? row.vendorId
          : "";
      $("#vendorId").val(vid);
      const rs = Array.isArray(row.roles) ? row.roles : [];
