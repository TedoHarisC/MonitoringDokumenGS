...
(function ($) {
  ...
  async function fetchJson(url, options) {
    ...
    if (!res.ok) {
      ...
      // Ensure we always have a plain-text message (guard against HTML error pages)
  ...
  function rolesToBadgeHtml(roles) {
    ...
    return rs
      .map(
        (r) =>
          `<span class="badge bg-light text-dark border me-1">${escapeHtml(r.code || r.name || "")}</span>`,
      )
      .join("");
  }
  ...
  async function openManageRolesModal(userId, username) {
    ...
    try {
      ...
      const currentRoleIds = normalizeToArray(userRoles).map((r) =>
        Number(r.roleId),
      );
  ...
  async function saveUserRolesFromModal() {
    ...
    const checkedIds = Array.from(
      document.querySelectorAll("#manageRoleList .role-check:checked"),
    ).map((x) => Number(x.value));

    const currentRoles = await fetchJson(`${apis.roles}/user/${userId}`);
    const currentIds = normalizeToArray(currentRoles).map((r) =>
      Number(r.roleId),
    );
    ...
    for (const roleId of toAdd) {
      await fetchJson(
        `${apis.roles}/assign?userId=${encodeURIComponent(userId)}&roleId=${encodeURIComponent(roleId)}`,
        { method: "POST" },
      );
    }
    ...
    for (const roleId of toRemove) {
      await fetchJson(
        `${apis.roles}/remove?userId=${encodeURIComponent(userId)}&roleId=${encodeURIComponent(roleId)}`,
        { method: "DELETE" },
      );
    }
  ...
  async function openAddAdminUserModal() {
    ...
    try {
      ...
      userSelect.innerHTML = users
        ...
        .map(
          (u) =>
            `<option value="${u.userId}">${escapeHtml(u.username)} — ${escapeHtml(u.vendorName || "")}</option>`,
        )
        .join("");

      const adminRoles = roles.filter((r) =>
        ADMIN_ROLE_CODES.includes((r.code || "").toUpperCase()),
      );
      const roleList = document.getElementById("addAdminRoleList");
      renderRoleCheckboxList(
        roleList,
        adminRoles,
  ...
  async function saveAddAdminUser() {
    ...
    const checkedIds = Array.from(
      document.querySelectorAll("#addAdminRoleList .role-check:checked"),
    ).map((x) => Number(x.value));
  ...
  function wireAdminUsersTab() {
    adminUsersTable = initTable(
      "#adminUsersTable",
      `${apis.roles}/admin-users`,
      [
        { data: "username" },
        { data: "email" },
        {
