  ...
  async function loadUangMukaDetail(id) {
    ...
    try {
      ...
      if (data.jenis === "Realisasi" && data.uangMukaRelatedId) {
        ...
        const relRes = await authFetch(
          `/api/uang-muka/${data.uangMukaRelatedId}`,
        );
        if (relRes.ok) {
          ...
          let relAttHtml = "";
          try {
            const relAttRes = await authFetch(
              `/api/attachments/by-reference/${rel.uangMukaId}`,
            );
            if (relAttRes.ok) {
              const relAtts = await relAttRes.json();
              if (Array.isArray(relAtts) && relAtts.length > 0) {
                relAttHtml =
                  `<div class=\"mt-2\"><b>Attachments Advanced:</b><ul>` +
                  relAtts
                    .map(
                      (a) =>
                        `<li><a href=\"/api/attachments/download/${a.attachmentId}\" target=\"_blank\" download>${a.fileName}</a></li>`,
                    )
                    .join("") +
                  `</ul></div>`;
              } else {
                relAttHtml =
                  '<div class=\"mt-2\"><b>Attachments Advanced:</b> <span class=\"text-muted\">No attachments</span></div>';
              }
            }
          } catch {}
      ...
      try {
        ...
        if (attRes.ok) {
          ...
          if (Array.isArray(atts) && atts.length > 0) {
            attachmentsHtml =
              `<div class="mb-3"><b>Attachments:</b><ul>` +
              atts
                .map(
                  (a) =>
                    `<li><a href="/api/attachments/download/${a.attachmentId}" target="_blank" download>${a.fileName}</a></li>`,
            ...
            attachmentsHtml =
              '<div class="mb-3"><b>Attachments:</b> <span class="text-muted">No attachments</span></div>';
          }
        }
      } catch {}
      ...
      function getStatusBadgeClass(status) {
        ...
        if (s.includes("done") || s.includes("selesai") || s.includes("lunas"))
          return "bg-success";
        if (
          s.includes("butuh") ||
      ...
      if (id)
        window.renderUangMukaDetailHistory &&
          window.renderUangMukaDetailHistory(id);
...
function loadUangMukaHistory(uangMukaId) {
  ...
  $.get(`/api/uang-muka/${encodeURIComponent(uangMukaId)}/history`)
    .done(function (data) {
      ...
      data.forEach(function (item) {
        ...
        const fallbackText = item.statusTransition
          ? `${item.statusTransition} oleh ${user} pada ${tgl}`
          : `Perubahan oleh ${user} pada ${tgl}`;

        html += `
                <div class="timeline-item mb-4">
                    <div class="d-flex align-items-center gap-2 mb-1">
                        <span class="badge bg-primary"></span>
                        <span class="text-muted small">${item.displayText || fallbackText}</span>
                    </div>
                </div>`;
      });
      html += "</div>";
